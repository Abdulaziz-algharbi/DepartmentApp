...
def send_issue_confirmation_email(issue: Issue) -> None:
    try:
        ...
        html_email = render_to_string(
            "emails/issue_confirmation.html", context)
        ...
        logger.error(
            f"Failed to send confirmation email for issue '{issue.title}': {e}", exc_info=True
        )
...
def send_issue_resolved_email(issue: Issue) -> None:
    try:
        ...
        logger.error(
            f"Failed to send resolution email for issue '{issue.title}': {e}", exc_info=True
        )
