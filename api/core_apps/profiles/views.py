...
class ProfileListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self) -> List[Profile]:
        return (
            Profile.objects.select_related("user").exclude(user__is_staff=True).exclude(
...
class AvatarUploadView(APIView):
    ...
    def upload_avatar(self, request, *args, **kwargs):
        ...
        # check if the file is actually send:
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"message": "Avatar uploading started."}, status=status.HTTP_202_ACCEPTED
            )
...
class NonTenantProfileListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self) -> List[Profile]:
        return (
            Profile.objects.select_related("user").exclude(user__is_staff=True).exclude(
                user__is_superuser=True).exclude(occupation=Profile.Occupation.Tenant)
        )
